...
- **Workout note** — structured per-exercise rows (name + set count, assumed 10 reps/set) that assemble into the same free-text entry the bot's `/workout` command writes; "+" adds another exercise row. Same edit-today/read-only-history pattern as weight and steps
