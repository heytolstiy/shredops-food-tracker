...
- **Existing project (if you previously ran the base schema):** paste and run `migration_water.sql`, then `migration_weight_log.sql`, then `migration_steps_log.sql`, then `migration_daily_targets.sql`
...
| `/week` | Sends a detailed 7-day report as a `.md` document — full per-meal log (with `meal_type`), daily calorie/macro adherence against the frozen target for each day, water, weight, steps, and weekly summary stats. Meant to be read by an external dietitian/trainer, not just the user |
