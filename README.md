...
├── migration_sleep_log.sql     # ALTER TABLE migration (adds sleep_logs)
├── package.json
...
- **Existing project (if you previously ran the base schema):** paste and run `migration_water.sql`, then `migration_weight_log.sql`, then `migration_steps_log.sql`, then `migration_daily_targets.sql`, then `migration_workout_log.sql`, then `migration_sleep_log.sql`
...
| `/sleep` | Record or view today's hours of sleep. `/sleep 7.5` upserts today's entry (overwrites, never duplicates); `/sleep` alone shows today's value |
| `/week` | Sends a detailed 7-day report as a `.md` document — full per-meal log (with `meal_type`), daily calorie/macro adherence against the frozen target for each day, water, weight, steps, sleep, workout notes, and weekly summary stats. Meant to be read by an external dietitian/trainer, not just the user |
...
- **Sleep card** — hours of sleep, same edit-today/read-only-history pattern as weight and steps
